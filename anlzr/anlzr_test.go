...
func TestRunAnalyzers(t *testing.T) {
	...
	if fmt.Sprintf("%.3f", res.AverageFuncLen) != "7.129" {
		t.Errorf("average_function_length: expected 7.129, found %.3f",
			res.AverageFuncLen)
	}
	...
	if res.TotalLoC != 111945 {
		t.Errorf("total_loc: expected 111945, found %d",
			res.TotalLoC)
	}
}
